...
describe("O servidor", () => {
  ...
  it("só cadastra monitor", () => {
    var options:any = {method: 'POST', uri: (base_url + "monitor"), body:{nome: undefined, chefe: false}, json: true};
  ...
  it("não cadastra monitor com nome duplicado", () => {
    return request.post(base_url + "monitor", {"json":{"nome": "Daniel", "disponibilidade":[false,false,false,false,true], "restricoes":[], "alocacoes":"0", "chefe":"false"}}).then(body => {
         ...
         return request.post(base_url + "monitor", {"json":{"nome": "Daniel", "disponibilidade":[false,false,false,false,true], "restricoes":[], "alocacoes":"2", "chefe":"false"}}).then(body => {
             expect(body).toEqual({failure: "O monitor não pode ser cadastrado"});
             return request.get(base_url + "monitores").then(body => {
                 ...
                 expect(body).not.toContain('[{"nome":"Daniel","disponibilidade":[false,false,false,false,true],"restricoes":[],"alocacoes":"2","chefe":"false"}]');
             });
         });
     });
  });
